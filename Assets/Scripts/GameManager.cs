...
public class GameManager : MonoBehaviour, IOnEventCallback, IInRoomCallbacks, IConnectionCallbacks, IMatchmakingCallbacks {
    ...
    public float tileWaterHeightMax, liquidRiseRate;
    public WaterSplash liquidToRise;
    public bool spawnRandomBombs = false;
    public MusicData mainMusic, invincibleMusic, megaMushroomMusic, overtimeMusic, whyDoIHearBossMusic, whyDoIHearFinalBossMusic;
    public bool WHYDOIHEARBOSSMUSIC, WHYDOIHEARFINALBOSSMUSIC;
    ...
    public bool paralizePlayer, allowRuning = true;
    ...
    {
        timedGameDuration = 1;
        int startTimestamp = startServerTime - 3500;
        endServerTime = startTimestamp + 4500 + timedGameDuration * 1000;
        endRealTime = startRealTime + 4500 + timedGameDuration * 1000;
    }
    ...
    {
        WHYDOIHEARBOSSMUSIC = b;
    }
    public void SetFinalBossMusic(bool b)
    {
        WHYDOIHEARFINALBOSSMUSIC = b;
    }
    ...
    private IEnumerator EndGame(Player winner) {
        ...
        //if(localPlayer && localPlayer.GetComponent<PlayerController>())
        //{
        //    localPlayer.GetComponent<PlayerController>().PlaySound(winner.IsLocal ? Enums.Sounds.Glados_Taunt_Win : Enums.Sounds.Glados_Taunt_Lose);
        //}
        if (draw)
        ...
        if(SingletonModeTag.mode == SingletonModeTag.Mode.Singleplayer)
        {
            if (win)
            {
                SaveManager.Instance.CompleteStage();
            }
    ...
    private IEnumerator OvertimeItemRain(bool wait = true) {
        foreach(StarBouncer star in FindObjectsOfType<StarBouncer>())
        {
            PhotonNetwork.Destroy(star.gameObject);
        }
        foreach(PlayerController con in FindObjectsOfType<PlayerController>())
        {
            con.photonView.RPC("ForceLivesTo", RpcTarget.All, 1);
        }
        float WaitTime = 1f;
        if (wait)
            yield return new WaitForSeconds(5); //wait for the beat drop

        if (!PhotonNetwork.IsMasterClient || gameover)
            yield break;

        bigwhile:
        while (true) { //todo: prevent bombs from spawning on players, and prevent entity limit capping
            Vector3 minPoint = Utils.TilemapToWorldPosition(new Vector3Int(levelMinTileX, levelMinTileY));
            Vector2 spawnPos = new Vector2(Random.Range(minPoint.x, minPoint.x + levelWidthTile), minPoint.y + (levelHeightTile / 2));
            if (spawnRandomBombs)
            {
                spawnPos.y = Random.Range(minPoint.y, minPoint.y + levelHeightTile);
            }
            foreach (var hit in Physics2D.OverlapCircleAll(spawnPos, 4))
            {
                if (hit.gameObject.CompareTag("Player"))
            ...
            if (Utils.IsTileSolidAtTileLocation(Utils.WorldToTilemapPosition(spawnPos)) && Utils.IsTileSolidAtWorldLocation(spawnPos))
            {
                if (wait)
                    yield return new WaitForSeconds(WaitTime);
                goto bigwhile;
            }
            PhotonNetwork.Instantiate("Prefabs/Enemy/Bomb", spawnPos, Quaternion.identity);
            if (wait)
                yield return new WaitForSeconds(WaitTime);
            if(WaitTime > .75f) //avoid crashing games
            {
                WaitTime -= .01f;
            }
    ...
    public void Update() {
        ...
        if (endServerTime != -1) {
            ...
            if (timeRemaining > 0 && !gameover) {
                ...
                if (!hurryup && (timeRemaining <= 60) && musicState != Enums.MusicState.FinalBoss) {
                    hurryup = true;
        ...
        if(musicState == Enums.MusicState.OVERTIME)
        {
            ...
            {
                liquidToRise.heightTiles += liquidRiseRate * Time.deltaTime;
            }
            else
            {
                liquidToRise.heightTiles = tileWaterHeightMax;
            }
        }
    }
    ...
    public void CheckForWinner() {
        ...
        if (alivePlayers.Count == 0) {
            ...
            {
                localPlayer.GetComponent<PlayerController>().photonView.RPC("PlaySound", RpcTarget.All, Enums.Sounds.Glados_Taunt_Lose);
            }
            PhotonNetwork.RaiseEvent((byte) Enums.NetEventIds.EndGame, null, NetworkUtils.EventAll, SendOptions.SendReliable);
            return;
            ...
            if (localPlayer && localPlayer.GetComponent<PlayerController>() && (localPlayer.GetComponent<PlayerController>().state == Enums.PowerupState.PortalGun || localPlayer.GetComponent<PlayerController>().state == Enums.PowerupState.Glockos))
            {
                localPlayer.GetComponent<PlayerController>().photonView.RPC("PlaySound", RpcTarget.All, (alivePlayers[0].gameObject == localPlayer ? Enums.Sounds.Glados_Taunt_Win : Enums.Sounds.Glados_Taunt_Lose));
            }
        ...
        if (timeUp) {
            ...
            {
                StartCoroutine(OvertimeItemRain(true));
            }
        ...
        if (starGame && winningStars >= starRequirement) {
            if (winningPlayers.Count == 1)
            {
                ...
                if (localPlayer && localPlayer.GetComponent<PlayerController>() && (localPlayer.GetComponent<PlayerController>().state == Enums.PowerupState.PortalGun || localPlayer.GetComponent<PlayerController>().state == Enums.PowerupState.Glockos))
                {
                    localPlayer.GetComponent<PlayerController>().photonView.RPC("PlaySound", RpcTarget.All, (winningPlayers[0].gameObject == localPlayer ? Enums.Sounds.Glados_Taunt_Win : Enums.Sounds.Glados_Taunt_Lose));
                }
    ...
    private void PlaySong(Enums.MusicState state, MusicData musicToPlay) {
        if (musicState == state)
            return;
        if(state == Enums.MusicState.OVERTIME)
        {
            onOvertime.Invoke();
        }
    ...
    private void HandleMusic() {
        ...
        float timeRemaining = (endServerTime - PhotonNetwork.ServerTimestamp) / 1000f;
        if (timeRemaining <= 0 && timedGameDuration > 0)
        {
            PlaySong(Enums.MusicState.OVERTIME, overtimeMusic);
        }
        else if (WHYDOIHEARBOSSMUSIC)
        {
            PlaySong(Enums.MusicState.BossMusic, whyDoIHearBossMusic);
        }
        else if (WHYDOIHEARFINALBOSSMUSIC)
        {
            PlaySong(Enums.MusicState.FinalBoss, whyDoIHearFinalBossMusic);
        }
        else if (mega)
        {
    ...
    public void Pause() {
        if (gameover || !musicEnabled || paralizePlayer)
            return;
    ...
    public void OnDrawGizmos() {
        ...
        if (liquidToRise)
        {
            ...
            Gizmos.DrawLine(new Vector3(GetLevelMinX(), tileWaterHeightMax / 2 + liquidToRise.transform.position.y), new Vector3(GetLevelMinX() + (levelWidthTile / 2f), tileWaterHeightMax / 2 + liquidToRise.transform.position.y));
        }
    ...
    public void WinPlayer(PlayerController con)
    {
        PhotonNetwork.RaiseEvent((byte)Enums.NetEventIds.EndGame, con.photonView.Owner, NetworkUtils.EventAll, SendOptions.SendReliable);
    }

    public void StartBossCam(Vector2 origin, float width)
    {
        useBossCam = true;
        bossCamOrigin = origin;
        bossCamWidth = width;
    }
    public void StartBossCam(Vector2 origin)
    {
        StartBossCam(origin, bossCamWidth);
    }
    public void StartBossCam()
    {
        StartBossCam(bossCamOrigin, bossCamWidth);
    }
}
