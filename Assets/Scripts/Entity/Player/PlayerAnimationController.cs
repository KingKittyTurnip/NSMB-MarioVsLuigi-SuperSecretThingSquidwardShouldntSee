...
public class PlayerAnimationController : MonoBehaviourPun {
    ...
    [SerializeField] private ParticleSystem dust, sparkles, drillParticle, giantParticle, fireParticle, dashReady, dashUsed;
    [SerializeField] public GameObject models, smallModel, largeModel, largeShellExclude, blueShell, propellerHelmet, propeller, glock, sword, bat, portalGun, goggles;
    ...
    public void HandleAnimations() {
        ...
        SetParticleEmission(dashReady, controller.state == Enums.PowerupState.Strawberry && !controller.dashed);
        SetParticleEmission(dashUsed, controller.state == Enums.PowerupState.Strawberry && controller.dashed);

        //Blinking
        if (controller.dead) {
            eyeState = Enums.PlayerEyeState.Death;
        } else {
            if (controller.state == Enums.PowerupState.Glock || controller.state == Enums.PowerupState.Sword || controller.state == Enums.PowerupState.Bat)
            {
    ...
    public void UpdateAnimatorStates() {
        ...
        animator.speed = controller.underWater ? .75f : 1f;
        ...
        animator.SetBool("underWater", controller.underWater);
        animator.SetBool("assIsOnFire", controller.buttBurnt > 0);

        float gunWeight = (controller.state == Enums.PowerupState.Glock || controller.state == Enums.PowerupState.PortalGun) && !(controller.wallSlideLeft || controller.wallSlideRight || controller.groundpound || controller.triplejump || controller.flying || controller.drill || controller.crouching || controller.sliding || controller.skidding) ? 1 : 0;
        animator.SetLayerWeight(1, gunWeight); //aim
        animator.SetLayerWeight(3, gunWeight); //additive fire and reload animations
        animator.SetLayerWeight(2, controller.state == Enums.PowerupState.Sword || controller.state == Enums.PowerupState.Bat ? 1 : 0); //for the animations where the player is holding the item behind them
        if (photonView.IsMine) {
            //Animation
            ...
            if (controller.stuckInBlock || (controller.pipeEntering && controller.pipeDirection.x == 0))
            {
                animatedVelocity = 0;
            }
            else if (controller.pipeEntering)
            {
                animatedVelocity = Mathf.Abs(controller.pipeDirection.x);
            }
            ...
            aim = Mathf.Lerp(aim, (controller.aim.y + 1) / 2, Time.deltaTime * 15);
            animator.SetFloat("velocityX", animatedVelocity);
            animator.SetFloat("forwardVelocity", body.velocity.x * (controller.facingRight ? 1 : -1));
            ...
            animator.SetBool("inShell", controller.inShell || (controller.CanShell() && (controller.crouching || controller.groundpound) && controller.groundpoundCounter <= 0.15f));
        } else {
        ...
        blueShell.SetActive(controller.inShell);
        ...
        glock.SetActive(controller.state == Enums.PowerupState.Glock);
        sword.SetActive(controller.state == Enums.PowerupState.Sword);
        bat.SetActive(controller.state == Enums.PowerupState.Bat);
        goggles.SetActive(controller.state == Enums.PowerupState.Goggles);
        portalGun.SetActive(controller.state == Enums.PowerupState.PortalGun);
    ...
    void HandleDeathAnimation() {
        ...
        if (deathTimer < deathUpTime) {
            ...
            if (!deathUp && body.position.y > GameManager.Instance.GetLevelMinY()) {
                body.velocity = new Vector2(0, deathForce * (controller.underWater ? .75f : 1));
                deathUp = true;
            ...
            body.gravityScale = 1.2f * (controller.underWater ? .75f : 1);
            body.velocity = new Vector2(0, Mathf.Max(-deathForce * (controller.underWater ? .75f : 1), body.velocity.y));
        }
        ...
        if (photonView.IsMine && deathTimer >= 3f * (SingletonModeTag.mode == SingletonModeTag.Mode.Singleplayer ? 2 : 1))
