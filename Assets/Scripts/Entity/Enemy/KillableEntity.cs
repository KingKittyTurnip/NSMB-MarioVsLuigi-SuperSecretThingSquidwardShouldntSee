...
public abstract class KillableEntity : MonoBehaviourPun, IFreezableEntity, ICustomSerializeView {
    ...
    public virtual void FixedUpdate() {
        if (!(photonView?.IsMine ?? true) || !GameManager.Instance || !photonView.IsMine || !body) //null propagation???
            return;
    ...
    [PunRPC]
    public virtual void SpecialKillWithForce(bool right, bool groundpound, int combo) {
        if (dead)
            return;
        ...
        body.constraints = RigidbodyConstraints2D.None;
        body.angularVelocity = 1000f * (right ? 1 : -1);
        body.gravityScale = 1.5f;
        audioSource.enabled = true;
        animator.enabled = true;
        hitbox.enabled = false;
        ...
        body.velocity = new(15f * (right ? 1 : -1), 15);
    }
