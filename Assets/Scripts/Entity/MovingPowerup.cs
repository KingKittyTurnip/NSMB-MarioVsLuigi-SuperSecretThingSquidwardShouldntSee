...
public class MovingPowerup : MonoBehaviourPun {
    ...
    private bool checkCollisionsThisFrame = true;
    [PunRPC]
    public void SmackWithForce(bool goRight) {

        body.velocity = new(15 * (goRight ? 1 : -1), 15f);
        checkCollisionsThisFrame = false;
    }

    public void HandleCollision() {
        if (!checkCollisionsThisFrame)
        {
            if(body.velocity.y < 0)
                checkCollisionsThisFrame = true;
            return;
        }
