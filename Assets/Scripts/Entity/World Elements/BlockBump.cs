...
public class BlockBump : MonoBehaviour {
    ...
    public void Start() {
        transform.position = new Vector3(transform.position.x, transform.position.y, .75f);
    ...
    public void Kill() {
        ...
        Matrix4x4 tileTransform = Matrix4x4.TRS(new Vector3(0, 0, 0), Quaternion.identity, Vector3.one);

        if (tile is AnimatedTile animatedTile)
        {
            tm.SetTile(loc, animatedTile);
            tm.SetTransformMatrix(loc, tileTransform);
        }
