...
public class GlobalController : Singleton<GlobalController>, IInRoomCallbacks, ILobbyCallbacks {
    ...
    public bool DevenUnlocked = false;
    ...
    public void Awake()
    {
        DevenUnlocked = PlayerPrefs.GetInt("DevenUnlocked", 0) == 1;
        if (!InstanceCheck())
            return;
    ...
    public void Update() {
        ...
        if (settings.ndsResolution && SceneManager.GetActiveScene().buildIndex != 0 && SceneManager.GetActiveScene().name != "WorldMap") {
